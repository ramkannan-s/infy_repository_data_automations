...
storage_quota_bytes="${6:?please enter the storage for project in bytes}"

project_template_cmd="cat projectTemplate.json | jq '.display_name = \"$projectname\"' | jq '.project_key = \"$projectid\"' | jq '.storage_quota_bytes = \"$storage_quota_bytes\"' > project.json"
