...
while IFS= read -r line; do
    if [[ $line == *"+"* ]]; then
        ...
        if [[ $data == "roles" ]]; then
            curl -XPOST -H "Authorization: Bearer ${JPD_AUTH_TOKEN}" "$TARGET_JPD_URL"/access/api/v1/projects/$projectname/$data -d @"$createdata.json" -s -H 'Content-Type: application/json'
        else
            curl -XPUT -H "Authorization: Bearer ${JPD_AUTH_TOKEN}" "$TARGET_JPD_URL"/access/api/v1/projects/$projectname/$data/$dataadd -d @"$createdata.json" -s -H 'Content-Type: application/json'
        fi
        echo -e "\n"
    elif [[ $line == *"-"* ]]; then
        ...
        curl -XDELETE -H "Authorization: Bearer ${JPD_AUTH_TOKEN}" "$TARGET_JPD_URL"/access/api/v1/projects/$projectname/$data/$datadelete
        echo -e ""
    else 
        echo -e "\nInvalid Input\n"
    fi
...
### sample cmd to run - ./getProjectComponentDiffConfigJPD.sh diffFile.txt http://35.208.78.203:8082 https://ramkannan.jfrog.io **** **** users/groups/roles <project_name>